...
@section('content')
    ...
    <div class="crud-edit-container">
        ...
        @elseif(str_contains(\Request::route()->getName(), '.create'))
            ...
            <form class="form-crud" method="post" action="{{ route('geographic-zones.store') }}">
                @csrf
        ...
        @elseif(str_contains(\Request::route()->getName(), '.edit'))
            ...
            <form class="form-crud" method="post" action="{{ route('geographic-zones.update', ['geographic_zone' => $id]) }}">
                @csrf
                @method('PUT')
                <label class="form-control required">
                    <span>{{ __('Name') }}</span>
