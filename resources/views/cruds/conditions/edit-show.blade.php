    ...
    <div class="crud-edit-container">
        ...
        @elseif(str_contains(\Request::route()->getName(), '.create'))
            ...
            @if($workflows->isEmpty())
                ...
                <p class="form-warning">{{ __('First create a new Workflow before creating a new Condition') }}.</p>

            @elseif($processes->isEmpty())

                <p class="form-warning">{{ __('First create a new Process before creating a new Condition') }}.</p>

            @else

                <form class="form-crud" method="post" action="{{ route('conditions.store') }}">
                    @csrf
        ...
        @elseif(str_contains(\Request::route()->getName(), '.edit'))
            ...
            <form class="form-crud" method="post" action="{{ route('conditions.update', ['condition' => $id]) }}">
                @csrf
                @method('PUT')
